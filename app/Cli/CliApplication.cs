...
public static class CliApplication
{
    public static async Task<int> RunAsync(string[] args)
    {
        ...
        return command switch
        {
            ...
            "native" => RunNative(commandArgs),
            _ => RunUnknownCommand(command)
        };
    }

    private static int RunNative(string[] args)
    {
        if (args.Length == 0 || IsHelp(args[0]))
        {
            WriteNativeHelp();
        ...
        {
            "prepare" => RunNativePrepare(args),
            _ => RunUnknownNativeCommand(args[0])
        };
    }
    ...
    private static int RunNativePrepare(string[] args)
    {
        if (args.Length > 1 && IsHelp(args[1]))
        {
            WriteNativeHelp();
...
""");
    ...
    private static void WriteNativeHelp()
    {
        Console.WriteLine($"""
...
""");
    ...
    private static int RunUnknownNativeCommand(string command)
    {
