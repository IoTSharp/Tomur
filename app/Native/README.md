...
Tomur-managed data/runtime directory. R3 prepares source bundle files from
`native/runtimes/<rid>/native` into
`<data>/runtime/<bundle-id>/<version>/runtimes/<rid>/native`, then probes and
loads only from that managed version directory.
