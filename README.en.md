...
| R15 | The M1-M7 foundation code is complete: the independent pure C# GLM / MoE provider library, provider selection boundary, model format probing, fixed-seed tiny fixture/oracle baseline, tensor storage/read layer, scalar reference kernels, tokenizer and incremental decoding, resident dense model loading, MLA reference/absorbed attention, interleaved partial RoPE, single-token decode, multi-token prefill, and compressed KV cache are in place. M8 MoE and expert streaming are next. Build, regression, oracle alignment, cross-platform, performance, and real-forward validation are deferred to M14. The existing llama.cpp path remains available and is still the default. |
