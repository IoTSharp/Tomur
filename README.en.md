...
| R15 | The M1-M9 foundation code is complete: the independent pure C# GLM / MoE provider library, provider selection boundary, model format probing, fixed-seed tiny fixture/oracle baseline, tensor storage/read layer, scalar reference kernels, tokenizer and incremental decoding, resident dense model loading, MLA with compressed KV cache, bounded MoE expert streaming, and bounded-batch full forward, prefill, decode, sampling, penalties, stops, context/cancellation handling, and incremental callbacks are in place. M10 Tomur API, session, streaming, and diagnostic integration is next. Build, regression, oracle alignment, cross-platform, performance, and real-model forward validation are deferred to M14. The existing llama.cpp path remains available and is still the default. |
