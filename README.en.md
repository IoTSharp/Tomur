...
| R15 | The M1-M10 foundation and M11 performance foundation are complete. The implementation includes explicitly marked `packed-offset` rowwise int4/int8 GLM weights, managed-model readiness, OpenAI and Anthropic SSE, incremental Ollama streaming, cancellable session unload, structured session/resource diagnostics, scalar-fallback SIMD/parallel matvec, paired gate/up dispatch, automatic expert cache/hot pin/prefetch behavior, forward-stage timing, an opt-in activation integer-dot evaluation kernel, prefill-wide expert union, and a switchable mmap I/O experiment boundary. Managed GLM now has explicit `glm4_moe_lite` architecture, configuration, and prompt contracts, while conversion, full-model loading, and conversation with real REAP weights remain pending on a separate machine. Random tiny GLM models have passed three compatibility API paths; the original BF16 `allenai/OLMoE-1B-7B-0125-Instruct` weights have passed Catalog discovery, provider loading, and real Chinese non-streaming conversation through the Ollama API. See the [packed GLM](./docs/r15-packed-glm-smoke.md), [GLM4 MoE Lite validation plan](./docs/r15-glm4-moe-lite-validation.md), and [OLMoE real-model smoke](./docs/r15-olmoe-smoke.md) records. The new M10/M11 regression code has not yet been built, tested, service-smoked, or performance-measured; benchmark, allocation, and cross-platform evidence remains part of M14. OLMoE OpenAI, Anthropic, SSE, full int8 conversion, and performance optimization also remain pending validation. Real conversation with the approximately 370 GB full GLM-5.2 model has not been run. The existing llama.cpp path remains available and is still the default. |
