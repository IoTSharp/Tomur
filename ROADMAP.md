...
6. 本阶段仍待用户明确要求后执行构建、启动和真实 GGUF smoke 验证；GPU offload、多模型常驻、reranker 和 R8 多模态不属于本次完成范围。
...
1. 在用户明确要求验证时执行 Tomur 独立项目构建、启动和真实 GGUF chat / embedding smoke。
2. 为 R7 增强逐 token streaming、GPU offload 选择、多模型常驻和更细的 session 诊断。
3. 为 R5 补充 Windows、Linux 和 macOS 实机安装/卸载/启动 smoke 验收记录。
4. 在 Tomur 单体内规划原生托盘外壳，不接线外部旧桌面项目。
