...
| 10 | R10 | 🚧 最小语音回合入口已接线 | 会话编排与语音回合服务 |
| 11 | R11 | 🚧 Runtime 操作区已接线 | React + Ant Design X Web UI |
...
11. R8 smoke 记录见 `Tomur/docs/r8-smoke-report.md`；当前结论为部分通过，图像生成仍是 R8 blocker，TTS 已完成真实合成适配但仍需补成功 smoke 证据；Windows CUDA13 构建与真实 GPU chat/offload smoke 仍需补证据。
...
8. ✅ Agent telemetry 草案：`GET /api/agents/telemetry` 暴露 `Tomur.Agents` `ActivitySource` 的 span / attribute 约定；agent chat、受控工具调用、只读 workflow 和失败路径会写入不含用户正文与完整工具结果的本地 Activity 标签，为后续 OpenTelemetry exporter 接入保留稳定边界。
9. ⏳ 图像生成工具：从会话中调用 `/v1/images/generations` 或内部 stable-diffusion.cpp adapter；当前手动入口为 `/v1/images/generations`，自动 tool-calling 仍留到 R9 后续受控调用流程，FLUX.2 成功出图 smoke 仍需补证据。
10. ⏳ 视觉理解工具：从会话中调用 VLM adapter，支持 data URI / base64 图片输入。
11. ⏳ OCR 工具：从会话中调用 OCR adapter，返回文本、版面和诊断信息。
12. ⏳ ASR 工具：从会话中调用 Whisper adapter，返回 transcript、语言、时间片段和诊断信息。
13. ⏳ TTS 工具：从会话中调用 llama.cpp GGUF TTS adapter，返回音频、音色、采样率和诊断信息；当前手动入口为 `/v1/audio/speech`，自动 tool-calling 仍留到 R9 后续受控调用流程。
14. ⏳ 文件问答工具：对接本地文件、SQLite 索引和基础 RAG，不引入 PostgreSQL 作为 Community 默认依赖。
15. ⏳ runtime 工具：允许会话读取 `doctor` / native / model readiness 诊断，但修复类动作必须有明确用户确认。
16. ⏳ Agent Framework telemetry 接入后续 OpenTelemetry exporter 管线，当前仅定义本地 ActivitySource 草案与事件日志边界。
...
8. `GET /api/agents/telemetry` 已接入 telemetry 草案读取；`AgentTelemetry` 使用 `System.Diagnostics.ActivitySource` 为 `agent_chat`、`tool_invocation`、`read_only_workflow` 与 agent error 路径打点，标签只记录 runtime、model、tool、status、elapsed、blocked、side effect 与诊断 code 等结构化字段。
...
10. R9 当前只代表 Microsoft AI 抽象、只读 AITool 绑定、受控只读工具调用、显式/自动只读工具上下文、本地事件摘要、结构化 agent 错误响应、本地 ActivitySource 草案和 Agent Framework 文本/只读 workflow 编排，不代表模型自动选择多模态 tool-calling、checkpoint、OpenTelemetry exporter 或本地文件 RAG 已完成。
...
3. `POST /api/conversations/{conversationId}/turns` 已接入基础文本回合编排：追加用户文本消息，调用本地 Agent Framework 文本路径，可按 `tool_mode` 执行 `runtime.diagnose` / `tools.inspect` 只读上下文，并把 assistant 回复、tool 摘要和失败诊断写回同一会话。
4. `POST /api/conversations/{conversationId}/voice-turns` 已接入最小语音回合入口：支持 multipart `file` 上传或 JSON `audio_base64` / `audio_data_uri`，先调用本地 ASR 得到 transcript，再复用文本回合编排生成 assistant 回复，默认尝试调用 TTS 并把输入音频与输出音频登记为会话产物；ASR、文本 runtime 或 TTS 缺失时会写入会话诊断，不伪造结果。
...
7. `POST /api/conversations/{conversationId}/diagnostics` 已支持记录会话级诊断、backend、model、动作建议和 metadata。
8. SQLite schema 已扩展到会话、消息、产物和诊断表，JSON 字段使用 source-generated serializer 边界；当前不代表自动多模态 tool-calling、文件 RAG、流式语音回合或前端历史同步已经完成。
...
7. 图片、文件和音频附件入口已在 Chat 输入区保留，但在相关后端能力与会话编排闭环前保持禁用，并显示明确诊断文案，不展示为已可用能力。
8. R11 当前代表 M0 工作台外壳、状态接线、Runtime 操作区和静态托管闭环，不代表完整 Settings 编辑能力、真实附件回合、语音模式或更完整的会话持久化已完成。
...
3. 🚧 停止生成、重新生成、复制和模型选择已在 Chat 工作台内完成；基础参数调整仍需后续补齐。
...
5. 在 Windows x64 CUDA 13 环境中执行 `tomur native build --rid win-x64 --backend all`，确认 llama.cpp / ggml、Whisper、PaddleOCR-VL、stable-diffusion.cpp 与 llama.cpp GGUF TTS 的 CPU 和 CUDA13 变体均能安装到 `native/runtimes/win-x64/native`。
6. 执行 `tomur native prepare`、`tomur doctor`、`GET /api/runtime/status` 与 `GET /api/runtime/multimodal`，确认 `ggml-cuda`、`whisper/cuda13`、`ocr/cuda13`、`stable-diffusion/cuda13`、`tts/cuda13` 被诊断为可见。
7. 补一次 Windows CUDA13 真实 chat smoke：下载推荐文本 GGUF，启动 Tomur，发起 `/v1/chat/completions`，记录 selected accelerator、GPU layers、token usage 和错误/成功证据。
8. 为 R7 增强多模型常驻、更细的 session 诊断、真实 CUDA / NPU offload smoke 和 Ollama 增量 streaming。
9. 在后期测试阶段补充 R5 的 Windows Service、Linux systemd、macOS launchd 和 Windows 托盘实机 smoke 验收记录。
10. 补齐并验证 macOS `osx-x64` / `osx-arm64` native runtime bundle 资产。
11. 继续推进 R9：在已接入 `runtime.diagnose` / `tools.inspect` 只读 AITool、`POST /api/agents/tools/invoke`、`tool_mode=auto_read_only`、`POST /api/agents/workflows/read-only`、`GET /api/agents/events`、`GET /api/agents/telemetry` 和统一 agent error JSON 的基础上，补构建/启动 smoke、后续 OpenTelemetry exporter 接入与受控 tool-calling 循环；等待 R8 图像与 TTS 成功 smoke 证据补齐后再开放对应自动生成工具。
12. 为 R10 继续收敛语音回合：补构建/启动 smoke、Web 录音入口、播放控制、失败诊断展示和后续流式 ASR/TTS 增强。
13. 为 R12 建立 AOT 审计清单，区分必须 AOT 的核心路径与可暂时走自包含 fallback 的高级编排路径。
