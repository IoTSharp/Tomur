...
| 07 | R7 | ✅ 已完成 | 本地推理首通 |
| 08 | R8 | ✅ 已完成 | 多模态能力 |
| 09 | R9 | 🚧 受控工具编排已接线 | Microsoft AI 抽象与 Agent Framework 编排 |
...
4. ✅ 受控工具调用入口：`POST /api/agents/tools/invoke` 可调用 `runtime.diagnose` 与 `tools.inspect`，也可在 `mode=controlled` 时调用 ready 的 R8 工具；`image.generate` 与 `audio.speak` 需要 `confirm=true`，生成产物写入 `<data>/files/agents`，边界阻塞返回 409，适配器诊断失败返回 503。
...
5. `POST /api/agents/tools/invoke` 已提供受控工具调用入口：默认只读模式执行 `runtime.diagnose` 与 `tools.inspect`，`mode=controlled` 可执行 ready 的 R8 adapters，并返回输入 schema、审计字段、source-generated JSON 结果、产物 metadata 或结构化诊断；`files.search` 和修复类 runtime 动作仍返回阻塞诊断。
...
10. R9 当前代表 Microsoft AI 抽象、只读 AITool 绑定、ready R8 controlled declaration、显式受控工具调用、显式/自动只读工具上下文、本地事件摘要、结构化 agent 错误响应、本地 ActivitySource 草案和 Agent Framework 文本/只读 workflow 编排；不代表模型自主选择多模态 tool-calling、checkpoint、OpenTelemetry exporter 或本地文件 RAG 已完成。

验收：

1. 普通文本会话不需要 Agent Framework 也能继续通过兼容 API 运行。
2. 🚧 `POST /api/agents/chat` 可以通过 `ChatClientAgent` 调用本地文本模型，并可显式或由 Tomur 自动规划注入只读工具上下文；显式 controlled tools 可调用 ready 的 R8 adapters；仍需构建/启动和真实模型 smoke 验证。
3. 🚧 `POST /api/agents/workflows/read-only` 可以执行受控只读工具步骤并可选摘要结果；仍需构建/启动和真实模型 smoke 验证。
4. 🚧 开启编排后，Tomur 可以在一次会话中按调用方显式工具请求调用图像生成、视觉理解、OCR、ASR、TTS 和 runtime 诊断工具；模型自主选择多模态工具仍未默认开放。
5. 🚧 工具调用失败时返回可诊断错误，不伪造图像、文字、音频或识别结果；R8 adapters 的真实模型 smoke 证据已记录，受控工具入口 smoke 归入 R9 后续验收。
...
继续推进 R9/R10，并维护已闭环的 R8 smoke 套件：
...
7. 继续推进 R9：在已接入 `runtime.diagnose` / `tools.inspect` 只读 AITool、ready R8 controlled declaration、`POST /api/agents/tools/invoke`、`tool_mode=controlled`、`tool_mode=auto_read_only`、`POST /api/agents/workflows/read-only`、`GET /api/agents/events`、`GET /api/agents/telemetry` 和统一 agent error JSON 的基础上，补构建/启动 smoke、后续 OpenTelemetry exporter 接入与模型自主工具选择循环。
8. 为 R10 继续收敛语音回合：补构建/启动 smoke、Web 录音入口、播放控制、失败诊断展示和后续流式 ASR/TTS 增强。
9. 为 R12 建立 AOT 审计清单，区分必须 AOT 的核心路径与可暂时走自包含 fallback 的高级编排路径。
