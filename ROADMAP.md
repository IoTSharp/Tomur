...
详细实现顺序、验证门槛、性能计划与发布标准见 [providers/Glm/ROADMAP.md](./providers/Glm/ROADMAP.md)。
...
5. 模型配置不兼容、张量缺失、量化格式未知、内存不足或 forward 尚未接通时返回结构化诊断，不伪造 token。
6. 独立托管程序集先面向非 AOT 自包含发布；Native AOT 的静态引用或不可用诊断在真实发布验证后决定。

首批交付：

1. 🚧 建立 provider 契约与选择边界，并用内置适配器保持 llama.cpp 现有行为。
2. 🚧 建立 `Tomur.Providers.Glm` 独立类库与中性 provider ID。
3. 🚧 实现模型目录、配置、tokenizer 与 safetensors header/tensor index 的只读探测。
4. 🚧 已建立固定 seed 的 tiny F32 fixture、版本化 oracle、tensor manifest、SHA-256 校验与隐藏生成/验证入口；尚待编译和回归执行。
5. 🚧 已建立统一 tensor descriptor、只读 shard 随机访问、resident/scratch 所有权、F32/F16/BF16 转换、int4/int8 量化视图与 expert slab；M3 编译和回归尚未执行。
6. ⏳ 实现量化矩阵乘、RMSNorm 与基础激活算子。
7. ⏳ 实现 MLA attention、压缩 KV cache、router、shared expert 与 routed expert forward。
8. ⏳ 实现按层 expert cache、RAM 配额、异步磁盘读取、取消和诊断。
9. ⏳ 使用 tiny oracle 对齐 tokenization、teacher forcing 与 greedy decode，再进行完整模型实验。

验收：

1. llama.cpp provider 的模型发现和调用路径保持不变。
2. 托管 provider 可以拒绝不匹配模型，并为缺失配置或张量返回明确诊断。
3. tiny 模型 forward 与独立 oracle 对齐前，不把该 provider 标记为可用于真实聊天。
4. 完整模型实验必须记录内存、磁盘读取、首 token 延迟、token/s 和输出正确性证据。
