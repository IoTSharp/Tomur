...
3. 请求已存在模型但本地推理未接通时返回 `runtime_not_configured`。
...
1. 为 R5 补充 Windows、Linux 和 macOS 实机安装/卸载/启动 smoke 验收记录。
2. 在 Tomur 单体内规划原生托盘外壳，不接线外部旧桌面项目。
3. R4 只能接入当前已经存在的 runtime 边界；真实文本推理仍属于 R7，本地多模态推理仍属于 R8。
4. 在 R7 前继续保持聊天端点的 `runtime_not_configured` 诊断，不伪造本地推理结果。
