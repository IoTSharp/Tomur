...
5. ⏳ 实现 int4/int8 张量视图、量化矩阵乘、RMSNorm 与基础激活算子。
6. ⏳ 实现 MLA attention、压缩 KV cache、router、shared expert 与 routed expert forward。
7. ⏳ 实现按层 expert cache、RAM 配额、异步磁盘读取、取消和诊断。
8. ⏳ 使用 tiny oracle 对齐 tokenization、teacher forcing 与 greedy decode，再进行完整模型实验。
