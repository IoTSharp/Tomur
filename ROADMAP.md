...
4. 在后期测试阶段补充 R5 的 Windows Service、Linux systemd、macOS launchd 和 Windows 托盘实机 smoke 验收记录。
5. 补齐并验证 macOS `osx-x64` / `osx-arm64` native runtime bundle 资产。
6. 为 R9 设计 Tomur native 能力到 Microsoft.Extensions.AI / Agent Framework tool 的映射表，先覆盖 chat、image、vision、OCR、ASR、TTS、files 和 runtime diagnostics。
7. 为 R10 设计语音回合的最小闭环：录音/上传、ASR、会话处理、TTS、播放和产物登记。
8. 为 R12 建立 AOT 审计清单，区分必须 AOT 的核心路径与可暂时走自包含 fallback 的高级编排路径。
