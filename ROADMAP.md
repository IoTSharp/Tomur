...
| 11 | R11 | 🚧 会话前端接线中 | React + Ant Design X Web UI |
...
9. SQLite schema 已扩展到会话、消息、产物和诊断表，JSON 字段使用 source-generated serializer 边界；当前不代表模型自主多模态 tool-calling、流式语音回合、VAD/打断或前端历史同步已经完成。
...
9. 纯文本普通对话仍保留 `/v1/chat/completions` streaming 路径；首次进入 `/api/conversations` 回合时会把当前前端内存中的纯文本历史补写到后端会话，避免附件/语音回合丢失前文。
10. R11 当前代表 M0 工作台外壳、状态接线、Runtime 操作区、附件回合、按钮式录音、TTS 播放和诊断展示已接通；不代表完整 Settings 编辑能力、下载队列、VAD/唤醒词/打断、流式语音回合或完整前端会话历史同步已完成。
...
2. 🚧 附件、按钮式录音、TTS 播放和会话诊断已接入 `/api/conversations`；完整历史列表同步、附件目录管理和更完整文件索引配置仍需后续补齐。
...
继续维护 R10 后端闭环，推进 R11/R12，并维护已闭环的 R8/R9 smoke 套件：
...
8. 为 R10 补构建/启动 smoke，并在 R11 接入 Web 录音入口、播放控制、失败诊断展示和后续流式 ASR/TTS 增强。
