...
1. 为 R4 增补 OpenAI / Ollama 缺失协议端点、错误模型和 streaming 骨架。
2. R4 只能接入当前已经存在的 runtime 边界；真实文本推理仍属于 R7，本地多模态推理仍属于 R8。
3. 在 R7 前继续保持聊天端点的 `runtime_not_configured` 诊断，不伪造本地推理结果。
