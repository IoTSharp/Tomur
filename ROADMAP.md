...
10. ⏭️ 实现按层 expert cache、RAM 配额、异步磁盘读取、取消和诊断。
11. ⏳ 使用 tiny oracle 对齐 tokenization、teacher forcing 与 greedy decode，再进行完整模型实验。

集中验证：

1. M1-M7 的编译、自动化测试、CLI/API 实跑和跨平台资源释放尚未执行，统一列入详细路线图 M14。
