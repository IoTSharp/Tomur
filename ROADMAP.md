...
7. ⏭️ 实现 tokenizer、prompt template 与增量 UTF-8 解码。
8. ⏳ 实现 MLA attention、压缩 KV cache、router、shared expert 与 routed expert forward。
9. ⏳ 实现按层 expert cache、RAM 配额、异步磁盘读取、取消和诊断。
10. ⏳ 使用 tiny oracle 对齐 tokenization、teacher forcing 与 greedy decode，再进行完整模型实验。

集中验证：

1. M1-M4 的编译、自动化测试、CLI/API 实跑和跨平台资源释放尚未执行，统一列入详细路线图 M14。
2. M5-M13 先完成基础代码；kernel/oracle 对齐、tokenizer、forward、API、性能、发布兼容与完整模型验证统一在 M14 执行。
