...
| 03 | R3 | 🚧 进行中 | Native Library Bundle |
...
12. ✅ `GET /api/runtime/native`、`GET /api/runtime/native/{componentId}/{libraryName}` 与 `POST /api/runtime/native/{componentId}/{libraryName}/load`。
...
3. ⏭️ 损坏的 native library 目前可通过 checksum / load failure 诊断，但还不会自动重新释放或修复。
4. ✅ Windows x64 和 Linux x64 native wrappers 已配置、编译、安装和 smoke-load 通过。
5. ⏭️ 完成版本化 runtime extraction 与修复动作后，R3 才能整体标记完成。
...
4. R4 可以基于现有 native probe / resolver / loader 开始协议面接线，但不能把 R3 剩余的 extraction / repair 当作已完成。
...
7. TTS：llama.cpp TTS / GGUF TTS bundle。
...
继续完成 R3 验证并为 R4 接口接线：

1. 完成版本化 runtime extraction、版本目录切换与损坏 native library 修复动作。
2. 为 R4 增补 OpenAI / Ollama 缺失协议端点、错误模型和 streaming 骨架。
3. R4 只能接入当前已经存在的 runtime 边界；真实文本推理仍属于 R7，本地多模态推理仍属于 R8。
