...
3. SQLite 初始化。
4. API key 本地存储。
5. `tomur doctor` 输出 OS、架构、CPU、内存、磁盘、proxy、端口、数据目录、runtime 状态。
...
1. 建立单个 `Tomur.csproj`。
2. 实现 `tomur --help`、`tomur serve`、`tomur doctor`。
3. 实现 `/health`、`/api/version`、`/v1/models`。
4. 为 OpenAI / Ollama 聊天端点返回清晰的未配置 runtime 诊断。
5. 按项目约束执行显式构建验证后，再将 R1 标记为完成。
