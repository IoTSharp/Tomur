...
- `tomur_sd_generate_png` 会转发 prompt、negative prompt、size、steps、CFG、seed、sampler、scheduler、finite `distilled_guidance` 和 finite `flow_shift`；调用方未指定 sampler / scheduler 时由上层决定默认值，无法识别时回到 upstream 默认值。
