...
构建产物安装到 `native/runtimes/win-x64/native`。随后执行 `tomur native prepare`，Tomur 会把这些资产复制到受管理 runtime 目录，并由 `tomur doctor`、`GET /api/runtime/status` 与 `GET /api/runtime/multimodal` 报告 CPU / CUDA13 可见性。

## ggml 隔离

`llama.native` 是顶层 `runtimes/<rid>/native/llama*` 与 `ggml*` 的唯一发布者。CUDA13 构建会把 `ggml-cuda` 作为可选 accelerator backend 发布到同一顶层目录；缺失时 CPU 运行时仍可诊断和加载。
