...
![OLMoE rowwise-int8 模型在 Tomur Web Chat 中完成非流式真实对话](./images/r15-olmoe-int8-web-chat.png)

图中选中的 `text/olmoe-1b-7b-0125-instruct-int8` 由 `managed-olmoe` provider 加载，页面展示本轮 Tomur Chat 非流式请求的真实模型回复。界面中的“待处理”诊断来自可读配置文件提示，不代表模型 forward 失败。
