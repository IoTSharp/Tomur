...
| R15 | M1-M6 基础代码已完成：独立类库、provider 选择边界、模型格式探测、固定 seed 的 tiny fixture / oracle、张量存储读取层、scalar reference kernels、tokenizer 与增量解码，以及 resident dense model 的 shape/dtype 校验、内存预预算、资源生命周期、embedding、input RMSNorm 与 dense MLP 基础路径均已落地；M7 MLA attention 与压缩 KV cache 为下一阶段。构建、回归、oracle 对齐、跨平台、性能与真实 forward 验证统一后置到 M14；现有 llama.cpp 路径继续保留并作为默认路径。 |
