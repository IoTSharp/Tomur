...
当前实现已扩展到 R2 配置、数据目录与诊断基础。`Tomur.csproj` 承载 CLI 与本地 HTTP API，提供 `tomur --help`、`tomur serve`、`tomur doctor`、`tomur api-key create/list`、`GET /health`、`GET /api/version`、`GET /api/runtime/status`、`GET /v1/models`、`POST /v1/chat/completions` 和 `POST /api/chat` 的基础协议面。
