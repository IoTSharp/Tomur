...
| R15 | M1-M10 基础代码和 M11 性能优化基础已完成。当前已接入显式 `packed-offset` rowwise int4/int8 GLM 权重、managed model readiness、OpenAI / Anthropic SSE、Ollama 增量 streaming、可取消 session unload、结构化 session/resource 诊断、可回退 scalar 的 SIMD/并行 matvec、gate/up paired dispatch、自动 expert cache/hot pin/prefetch、forward 阶段 timing、activation integer dot 评估、prefill batch expert union 和可切换 mmap I/O 实验边界。真实 REAP 权重的转换、完整模型加载和对话验证仍待异机执行。随机 tiny GLM 已完成三类兼容 API 链路 smoke；`allenai/OLMoE-1B-7B-0125-Instruct` 原始 BF16 权重已通过 Catalog、provider load 和中文 Ollama 非流式真实对话，证据见 [packed GLM](./docs/r15-packed-glm-smoke.md)、[GLM4 MoE Lite 异机验证计划](./docs/r15-glm4-moe-lite-validation.md) 与 [OLMoE real-model smoke](./docs/r15-olmoe-smoke.md)。新增 M10/M11 回归代码尚未执行构建、测试、服务 smoke 或性能测量；全部性能基准、allocation 和跨平台验证归 M14。OLMoE 的 OpenAI、Anthropic、SSE、完整 int8 转换和性能优化仍待验证。约 370 GB 的完整 GLM-5.2 模型尚未执行真实对话。现有 llama.cpp 路径继续保留并作为默认路径。 |
