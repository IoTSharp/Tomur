...
| R15 | M1-M5 基础代码已完成：独立类库、provider 选择边界、模型格式探测、固定 seed 的 tiny fixture / oracle、张量存储读取层、scalar reference kernels、tokenizer、GLM prompt template 与增量 UTF-8/stop 解码均已落地，M6 resident dense model 加载为下一阶段。构建、回归、oracle 对齐、跨平台、性能与真实 forward 验证统一后置到 M14；现有 llama.cpp 路径继续保留并作为默认路径。 |
