...
| R15 | M1-M9 基础代码已完成：独立类库、provider 选择边界、模型格式探测、固定 seed 的 tiny fixture / oracle、张量存储读取层、scalar reference kernels、tokenizer 与增量解码、resident dense model、MLA 与压缩 KV cache、MoE expert streaming，以及有界批次完整 forward、prefill、decode、sampling、penalty、stop、context/cancellation 和增量 callback 均已落地；M10 Tomur API、session、streaming 与诊断闭环为下一阶段。构建、回归、oracle 对齐、跨平台、性能与真实模型 forward 验证统一后置到 M14；现有 llama.cpp 路径继续保留并作为默认路径。 |
