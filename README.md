...
Web 工作台采用 React、TypeScript、Vite、`antd` 与官方 `@ant-design/x` 技术栈。默认入口采用 Chat-first 信息架构；模型、下载、运行时和文件相关管理能力优先收敛在 Settings 分组、状态抽屉或 Chat 上下文诊断入口中。前端构建产物由 Tomur 本地 HTTP 服务托管。
