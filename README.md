...
多模态能力处于 R8 接线阶段。`GET /api/runtime/multimodal` 会报告 Whisper ASR、llama.cpp GGUF TTS、PaddleOCR-VL、stable-diffusion.cpp 和 llama.cpp VLM 的 native component 与本地模型资产可见性；`/v1/audio/transcriptions` 已通过 Whisper large-v3 turbo q5_0 真实模型 smoke；`/api/ocr/analyze` 以及包含 data URI / base64 图片的 `/v1/chat/completions` 已通过 SmolVLM 快速 smoke。`/v1/audio/speech` 已接到 OuteTTS GGUF bundle 和 `tomur-tts` ABI，但 native bridge 仍返回 `pending-llama-tools-tts-adapter`，尚未生成音频。`/v1/images/generations` 已改为通过 Tomur 内部 image worker 子进程调用 stable-diffusion.cpp，native assert、超时或 worker 崩溃会回到主进程的结构化诊断；FLUX.2 klein 成功出图仍被 stable-diffusion.cpp conditioner assert 阻塞。R8 smoke 证据见 [docs/r8-smoke-report.md](docs/r8-smoke-report.md)。

R9 已开始接入 Microsoft AI 抽象与 Agent Framework 编排。Tomur 本地文本 runtime 已暴露为 `Microsoft.Extensions.AI.IChatClient`，`POST /api/agents/chat` 通过 `Microsoft.Agents.AI.ChatClientAgent` 执行本地纯文本会话；`GET /api/agents/runtime` 与 `GET /api/agents/tools` 提供 chat、image、vision、OCR、ASR、TTS、files 和 runtime diagnostics 的工具状态映射，`GET /api/agents/tool-bindings` 会暴露当前可解析的 `Microsoft.Extensions.AI.AITool` 绑定。当前 R9 只启用纯文本编排、工具目录和只读诊断类 AITool，不默认开放自动 tool-calling；TTS 与图像生成仍按 R8 blocker 处理。
