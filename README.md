...
| R13 | Web 前端能力聚合闭环已接入 Agent / Capabilities 聚合视图、只读 Agent 工具入口、协议能力地图和 Claude Code / Anthropic Messages 兼容协议面；副作用确认流、可视化下载队列和 Settings 写入仍在推进。 |

仍属于后续工作的内容包括：副作用工具确认流、可视化下载队列、Settings 写入编辑、模型删除、VAD / 打断、流式语音回合、多模型常驻、Linux/macOS 发布执行记录和服务形态实机 smoke。
