...
| R15 | M1-M7 基础代码已完成：独立类库、provider 选择边界、模型格式探测、固定 seed 的 tiny fixture / oracle、张量存储读取层、scalar reference kernels、tokenizer 与增量解码、resident dense model，以及 MLA reference/absorbed attention、interleaved partial RoPE、单 token decode、多 token prefill 和压缩 KV cache 均已落地；M8 MoE 与 expert streaming 为下一阶段。构建、回归、oracle 对齐、跨平台、性能与真实 forward 验证统一后置到 M14；现有 llama.cpp 路径继续保留并作为默认路径。 |
