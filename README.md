...
| R15 | M1-M9 基础代码已完成；M10 集成正在推进，已接入显式 `packed-offset` rowwise int4/int8 GLM 权重与独立 `managed-olmoe` provider。随机 tiny GLM 已完成三类兼容 API 链路 smoke；`allenai/OLMoE-1B-7B-0125-Instruct` 原始 BF16 权重已通过 Catalog、provider load 和中文 Ollama 非流式真实对话，证据见 [packed GLM](./docs/r15-packed-glm-smoke.md) 与 [OLMoE real-model smoke](./docs/r15-olmoe-smoke.md)。OLMoE 的 OpenAI、Anthropic、SSE、完整 int8 转换和性能优化仍待验证；约 370 GB 的完整 GLM-5.2 模型尚未执行真实对话。现有 llama.cpp 路径继续保留并作为默认路径。 |
