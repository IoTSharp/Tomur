...
当前实现已完成 R1 单项目 API 骨架与 R2 配置、数据目录、SQLite、API key 和诊断基础。R3 已完成 native source bundle 迁移、manifest、probe、resolver、loader、Windows/Linux wrapper 构建验证和 load smoke 边界，但版本化 runtime extraction 与损坏自动修复仍未完成。

`Tomur.csproj` 承载 CLI 与本地 HTTP API，提供 `tomur --help`、`tomur serve`、`tomur doctor`、`tomur api-key create/list`、`GET /health`、`GET /api/version`、`GET /api/runtime/status`、`GET /api/runtime/native`、`GET /api/runtime/native/{componentId}/{libraryName}`、`POST /api/runtime/native/{componentId}/{libraryName}/load`、`GET /v1/models`、`POST /v1/chat/completions` 和 `POST /api/chat` 的基础协议面。
...
Tomur 发行产物应包含本地 AI backend 所需的 native dynamic libraries。当前 R3 能通过 manifest 探测、checksum、路径解析和显式 load smoke 报告缺失或无法加载的 native library；受管理 runtime 目录的版本化释放、更新和损坏自动修复仍是 R3 剩余工作。
