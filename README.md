...
| R15 | M1-M3 基础代码已完成：独立类库、provider 选择边界、模型格式探测、固定 seed 的 tiny fixture / oracle 基线与张量存储读取层均已落地，M4 scalar reference kernels 为下一阶段。构建、回归、oracle 对齐、跨平台、性能与真实 forward 验证统一后置到 M14；现有 llama.cpp 路径继续保留并作为默认路径。 |
