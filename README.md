...
Tomur 是一个集成了本地模型服务、OpenAI / Ollama 兼容 API、模型资产管理、运行时诊断和 Chat-first Web 工作台的本地 AI 基础设施，面向离线优先、隐私敏感、低运维成本的个人与团队开发环境。它基于 .NET 10 与 C# 构建，以单一 `tomur` 进程承载 CLI、本地 HTTP 服务、系统服务形态、native runtime 管理和 Web 静态资源托管。
...
1. 🔌 启动一个本地服务，提供 OpenAI / Ollama 兼容 API。
...
7. ⚙️ llama.cpp、Whisper、OCR native、stable-diffusion.cpp 与 llama.cpp TTS / GGUF TTS native runtime 支持。
8. 🖥️ 系统服务运行模式。
9. 🧑‍💻 React + Ant Design X Web 工作台。

Tomur 不会在未接通本地 runtime 时伪造推理结果。模型缺失、native runtime 不可用、bundle 资产损坏、上下文超限、能力不匹配或内存不足时，API、CLI 和 UI 都应返回可诊断的错误。
...
Tomur 已完成 R1 至 R11 的主要闭环，并进入 R12 Native AOT / 自包含发布矩阵收敛阶段。
...
| R13 | Web 前端能力聚合闭环开始推进；目标是在保持 Chat-first 入口的前提下聚合 Agent、协议、多模态、产物、文件检索和 Settings 写入能力。 |

仍属于后续工作的内容包括：Agent / Capabilities 前端聚合、可视化下载队列、Settings 写入编辑、模型删除、VAD / 打断、流式语音回合、多模型常驻、Linux/macOS 发布执行记录和服务形态实机 smoke。
...
`Tomur.csproj` 承载 CLI、本地 HTTP API、服务模式启动、native runtime 管理和 Web 静态资源托管。`Program.cs` 负责进程入口、顶层命令分发和全局帮助；具体 CLI 实现按类别放在 `app/Cli/`。

`native/` 放置 native backend 源码、CMake 工程和发布打包边界；`app/Native/` 只保留 C# 动态库加载、P/Invoke 与托管适配代码。Web 源码位于 `web/`，构建产物输出到 `app/wwwroot`，运行时由 Tomur 本地 HTTP 服务托管。
...
`--backend cpu` 与 `--backend cuda13` 可只构建单一变体。CUDA、NPU、Metal、Vulkan、SYCL、OpenVINO 等 accelerator backend 按可见性和 native runtime 支持进入 offload 策略；缺失时应回退 CPU 路径。

## 🗺️ 路线图

长期阶段计划、完成口径和后续工作统一维护在 [ROADMAP.md](./ROADMAP.md)。README 只保留项目首页所需的定位、使用路径和当前边界。
