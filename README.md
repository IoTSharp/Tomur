...
| R15 | M1-M9 基础代码已完成；M10 集成正在推进，已接入显式 `packed-offset` rowwise int4/int8 GLM 权重、独立 `managed-olmoe` provider、managed model readiness、Ollama 增量 streaming、可取消 session unload 和结构化资源诊断。managed GLM 已增加显式 `glm4_moe_lite` architecture/config/prompt 契约，但真实 REAP 权重的转换、完整模型加载和对话验证仍待异机执行。随机 tiny GLM 已完成三类兼容 API 链路 smoke；`allenai/OLMoE-1B-7B-0125-Instruct` 原始 BF16 权重已通过 Catalog、provider load 和中文 Ollama 非流式真实对话，证据见 [packed GLM](./docs/r15-packed-glm-smoke.md)、[GLM4 MoE Lite 异机验证计划](./docs/r15-glm4-moe-lite-validation.md) 与 [OLMoE real-model smoke](./docs/r15-olmoe-smoke.md)。新增 M10 readiness、streaming 与 unload 代码尚未执行构建、测试和服务 smoke；OLMoE 的 OpenAI、Anthropic、SSE、完整 int8 转换和性能优化仍待验证。约 370 GB 的完整 GLM-5.2 模型尚未执行真实对话。现有 llama.cpp 路径继续保留并作为默认路径。 |
