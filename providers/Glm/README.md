...
The M1-M5 foundation code is in place: model metadata probing, safetensors indexing, fixed-seed tiny oracle fixture generation, bounded tensor storage, numeric conversion, quantized views, workspaces, expert slabs, scalar reference kernels, tokenizer pipelines, GLM prompt construction, and incremental UTF-8/stop decoding. M6 resident dense model loading is next. Build, regression, oracle alignment, cross-platform, performance, and release validation are intentionally deferred to the final M14 validation stage; forward execution remains unavailable until that stage passes.
