...
The M1-M8 foundation code is in place: model metadata probing, safetensors indexing, fixed-seed tiny oracle fixture generation, bounded tensor storage, numeric conversion, scalar reference kernels, tokenizer and prompt pipelines, resident dense model loading, MLA reference/absorbed attention, compressed KV cache, MoE routing, shared experts, and bounded expert streaming with per-layer leased LRU slots. M9 full forward and generation are next. Build, regression, oracle alignment, cross-platform, performance, and release validation are intentionally deferred to the final M14 validation stage; forward execution remains unavailable until that stage passes.
