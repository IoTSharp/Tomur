...
The M1-M6 foundation code is in place: model metadata probing, safetensors indexing, fixed-seed tiny oracle fixture generation, bounded tensor storage, numeric conversion, quantized views, workspaces, expert slabs, scalar reference kernels, tokenizer pipelines, GLM prompt construction, incremental UTF-8/stop decoding, and resident dense model loading with memory preflight and explicit resource ownership. The resident path includes embedding, input RMSNorm, and dense MLP scalar foundations; M7 MLA attention and compressed KV cache are next. Build, regression, oracle alignment, cross-platform, performance, and release validation are intentionally deferred to the final M14 validation stage; forward execution remains unavailable until that stage passes.
