...
当前状态：🚧 已开始部分验证。P0 已将生产 MLA 默认模式切换到 Absorbed，Reference 继续作为显式 oracle 路径；本机 M7 7/7、M9 6/6 与 Linux 隔离副本 M7 6/6、M9 6/6 已通过。完整 GLM-4.7 固定 1-token completion 从 Reference `186.596971s` 降至 Absorbed `26.595764s`，返回相同 token，端到端改善 `7.02x`。该结果不代表 M14 其余构建、协议、性能、资源和跨平台矩阵完成。
